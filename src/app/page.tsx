  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-10">
      <motion.section variants={item} className="text-center">
        <p className="mb-2 text-sm font-medium uppercase tracking-widest text-[var(--foam)]">
          Ocean Middle School
        </p>
        <h1 className="mb-4 text-4xl font-bold text-[var(--foam-light)] sm:text-5xl">
          오션중 학생 허브
        </h1>
      ...
      <motion.section variants={item} className="glass-card p-6 sm:p-8">
        ...
        {list.length === 0 ? (
          <p className="text-sm text-[var(--text-muted)]">예정된 일정이 없습니다. src/data/events.json 을 수정하세요.</p>
        ) : (
          <ul className="space-y-3">
            {list.map((e) => (
              <li key={e.date + e.title} className="flex flex-wrap items-baseline justify-between gap-2 border-b border-[var(--surface-border)] pb-3 last:border-0 last:pb-0">
